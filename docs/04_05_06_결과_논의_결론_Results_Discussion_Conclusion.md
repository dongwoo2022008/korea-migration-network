...
둘째, 공간 패널모형(SDM, Dynamic FE-GMM) 분석 결과, 지역의 네트워크 중심성(PageRank)은 단순한 통계적 지표를 넘어 미래의 인구 유입을 견인하는 실질적 동인으로 작용함을 확인하였다(β=0.312, p<0.001). 이는 인구이동에 강한 경로의존성이 존재하며, 허브 지역이 주변 지역에 미치는 공간적 파급효과(Spatial Spillover)가 통계적으로 유의미함을 보여준다.
...
인구이동의 공간적 파급효과와 경로의존성이 입증된 만큼, 개별 시군구 단위의 분절적인 인구 유치 경쟁(Zero-sum Game)은 지양되어야 한다. 대신, 거점 허브 도시를 중심으로 주변 지역이 연계되는 광역적 네트워크 발전 전략이 필요하다. 중앙정부는 초광역권(Mega-region) 단위의 인프라 투자와 산업 배치를 통해 허브의 긍정적 파급효과(Spread Effect)가 주변부로 확산될 수 있도록 유도해야 한다.
