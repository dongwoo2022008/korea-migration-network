...
전통적 계량모형의 선형성 가정을 극복하고 복잡한 비선형 상호작용을 포착하기 위해 7개의 알고리즘을 비교 평가한다 (Table 3-8). 패널 데이터의 특성상 발생할 수 있는 데이터 누수(Data Leakage)를 방지하기 위해, 본 연구는 시계열 순서를 보존하는 **5-Fold Expanding Window Cross-Validation**을 적용하였다 [6]. 2017~2024년(8년) 데이터를 시간 축을 기준으로 분할하여, 초기 학습 데이터를 점진적으로 확장하며 성능을 평가한다. 구체적으로 첫 번째 Fold는 2017년 데이터로 학습하고 2018년을 검증하며, 마지막 5번째 Fold는 2017~2021년 데이터로 학습하고 2022년을 검증한다. 이 방식은 미래 정보가 과거 모델에 유입되는 것을 차단하고, 모델이 시간에 따른 분포 변화(Concept Drift)에 얼마나 견고한지 평가할 수 있다. 교차검증이 끝난 후, 전체 데이터(2017~2022)로 최종 모델을 재학습하고, Hold-out Test Set(2023~2024)에 대해 최종 성능을 평가하였다. 본 연구에서는 Optuna 프레임워크를 활용하여 모형별 80 trial의 하이퍼파라미터 탐색을 수행하였으며, 이를 통해 핵심 모형들의 최적 성능을 비교하여 데이터의 본질적인 비선형 패턴을 가장 잘 포착하는 최적 모형을 선정하였다.
...
실제 분석 결과(LightGBM, Track C), **노후주택비율(house_age)**의 Mean |SHAP| 값이 3.662로 1위를 차지하였으며, **네트워크 중심성(pagerank_lag1)**이 3.434로 2위를 기록하였다. 이어 보육시설(3.244), 인구밀도(2.757), 서울 거리(1.976) 순으로 나타났다. 네트워크 허브 지위가 주거·생활SOC 변수와 함께 인구 흡인의 핵심 결정요인임을 확인하였다.
...
| **Demo** | 인구 | `youth_ratio` | 청년비율 20섴39세 (%) |
...
산출된 RAI의 타당성은 다음 절차로 검증하였다. 첫째, RAI가 실제 순이동률(net_rate) 및 PageRank와 유의한 양(+)의 상관을 갖는지를 분석하여 수렴타당성(convergent validity)을 확인하였다. 분석 결과, RAI는 지역 평균(N=228, 2017~2024년 평균; 창원시 독립변수 결측으로 제외) 기준 1기 시차 PageRank(r=0.222, p<0.001)와 유의한 양(+)의 상관을 보였다. 순이동률과의 상관 또한 유의한 약한 양(+)의 상관(r=0.163, p=0.014)을 보여 수렴타당성이 확인되었다. 상관 크기가 약한 수준에 그치는 것은 RAI가 구조적 매력도(장기 평균)를 측정하는 반면, 순이동률은 신도시 입주·재개발 등 단기 변동성에 크게 영향받기 때문이며, 선행연구(Niedomysl, 2010; Haase et al., 2021)에서도 매력도 지수와 이동률 간 상관이 0.2~0.4 수준임을 보고한 바 있다. 둘째, RAI는 내적 일관성(Cronbach's α)이 요구되는 반영지표(Reflective Indicator)가 아니라 개별 결정요인들의 SHAP 기여가 합산되어 전체 매력도를 구성하는 형성지표(Formative Indicator)이므로, 단일 차원의 내적 일관성보다는 위에서 확인된 결과 변수(순이동률)와의 예측 타당성(Predictive Validity)이 지수의 신뢰도를 담보한다. 셋째, 주성분분석(PCA)을 수행하여 도출된 제1주성분(전체 분산의 56.4% 설명) 기반 가중 합성지수와 본 연구의 RAI 간 순위 상관(Spearman ρ)을 분석한 결과, ρ=0.998 (p<0.001)로 거의 완전한 일치를 보여 지수 산출 방식에 따른 결과의 강건성이 확인되었다.
...
**③ 기간분할 SHAP**: Track C 데이터(2017~2024)를 팬데믹 전(2017~2020)과 팬데믹 후(2021~2024)로 분할하여 LightGBM+SHAP을 각각 산출한다. 두 시기의 변수 중요도 순위 변화를 비교함으로써 팬데믹이 결정요인 구조에 미친 영향을 탐색적으로 분석한다. 표본 규모 제한(각 N≈887)으로 인해 탐색적 분석으로 해석한다.
