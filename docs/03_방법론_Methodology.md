...
전통적 계량모형의 선형성 가정을 극복하고 복잡한 비선형 상호작용을 포착하기 위해 7개의 알고리즘을 비교 평가한다 (Table 3-8). 패널 데이터의 특성상 발생할 수 있는 데이터 누수(Data Leakage)를 방지하고 시간적 인과성을 엄밀히 유지하기 위해, 본 연구는 데이터를 시간 축에 따라 분할하는 **Time-Series Split (Train/Validation/Test Split)** 방식을 적용하였다 [6]. 구체적으로, 전체 8년(2017~2024년)의 패널 데이터 중 과거 6년(2017~2022년)을 훈련 데이터(Train Set)로, 2023년을 하이퍼파라미터 튜닝을 위한 검증 데이터(Validation Set)로, 그리고 가장 최근 연도인 2024년을 최종 성능 평가를 위한 독립된 테스트 데이터(Test Set)로 분할하였다. 이러한 시계열 분할 방식은 미래의 정보가 과거 모델 학습에 참조되는 미래 누수(Future Leakage)를 원천적으로 차단한다. 하이퍼파라미터 튜닝은 Optuna 기반 베이지안 최적화를 Validation Set에 적용하여 수행하였다.
