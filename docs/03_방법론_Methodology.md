...
연령대별 인구이동 네트워크의 구조적 이질성을 분석하기 위해, 전체 OD 데이터를 연령군별로 분리하여 각각의 방향성 가중 네트워크를 구성한다. 연령군은 **청년(20–39세)**, **중장년(40–59세)**, **고령(60세 이상)**의 3개 집단으로 정의하며, 이는 생애주기별 이동 동기의 차이를 포착하기 위한 이론적 구분이다.

**데이터 구성:** 통계청 국내인구이동통계의 연령별(10세 단위) 전입·전출 데이터를 활용하여, 청년군은 20대+30대, 중장년군은 40대+50대, 고령군은 60대+70대이상을 합산한다. 분석 기간은 2008~2025년(18년)이며, 229개 시군구 간 OD 행렬을 연령군별로 구성한다(자기루프 제거, 크로스워크 통일 적용).

**비교 지표:** 각 연령군 네트워크에 대해 다음 지표를 산출하여 비교한다.

| 지표 | 측정 방법 | 해석 |
|:---|:---|:---|
| 총이동량 | 연령군별 OD 합산 | 이동 규모의 연령별 분화 |
| 네트워크 밀도 | 실제 엣지 수 / 최대 가능 엣지 수 | 네트워크 연결 정도 |
| HHI (PageRank) | $\sum p_i^2$ | 허브 집중도 |
| Top 10% 점유율 | 상위 10% 노드의 PageRank 합 | 소수 허브 쏠림 정도 |
| Modularity | Louvain 알고리즘 (random_state=42) | 권역 구조 강도 |
| 실질 커뮤니티 수 | Louvain 탐지 커뮤니티 수 | 이동 권역 분화 |

**분석 방법:** 2025년 기준 단면 비교(Table R3-1)와 2008~2025년 시계열 추이(Figure R3-1) 분석을 병행한다. 허브 구성의 생애주기별 분화는 연령군별 PageRank Top5 비교(Table R3-2)를 통해 확인한다.
