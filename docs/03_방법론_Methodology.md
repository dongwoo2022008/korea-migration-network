...
**[Model 2] Spatial Durbin Model (SDM, 강건성 검증):** Moran's I 검정을 통해 잔차의 공간 자기상관성이 확인되면 공간 패널 모형을 적용한다. SDM은 독립변수의 공간 시차항도 포함하여 파급효과(Spillover Effect)를 가장 유연하게 포착하는 모형이다 [4].

**[Model 3] Dynamic FE-GMM (System GMM, 주모형):** 공간적 의존성과 동태성을 동시에 추정하는 모형의 계산상 제약을 고려하여, 본 연구는 종속변수의 시차항($y_{i,t-1}$)을 포함하여 동태성과 내생성을 통제하는 Dynamic FE-GMM을 주모형으로 사용하고, 앞선 SDM을 공간적 강건성 검증 모형으로 활용한다. 본 연구는 FE, 공간패널모형, Dynamic FE-GMM을 단계적으로 적용함으로써 관측되지 않는 지역 특성, 공간적 의존성, 그리고 잠재적 내생성을 순차적으로 통제하고자 하였다.
