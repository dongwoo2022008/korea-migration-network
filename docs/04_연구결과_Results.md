...
본 절에서는 229개 시군구 패널 데이터(2009~2024년, N=3,552)를 활용하여 네트워크 중심성(PageRank)이 지역의 순이동률에 미치는 영향을 패널모형과 공간계량모형을 통해 분석한다. 모형의 내생성 통제를 위해 핵심 설명변수인 `ln(PageRank+1)`은 1년 시차(lag1) 변수로 투입하였으며, 인구 규모 등 비대칭성이 큰 통제변수는 자연로그로 변환하였다.

### 4.3.1 패널 고정효과 모형 추정 결과
...
지역 간 인구이동은 인접 지역의 경제·사회적 여건에 영향을 받는 공간적 파급효과(Spatial Spillover)를 동반한다. 이를 확인하기 위해 Two-way FE 모형의 잔차에 대해 Moran's I 검정과 LM 검정을 수행한 결과, 고정효과가 공간 구조를 상당 부분 흡수하여 잔차의 공간 자기상관은 약하게 나타났다. 그러나 본 연구는 인구이동의 본질적인 공간적 파급 경로를 규명하기 위해 공간더빈모형(Spatial Durbin Model, SDM)을 추정하였다(Table 4-13). 공간 가중치 행렬(W)은 직접 인접성(Queen contiguity) 기준을 적용하였으며, LeSage and Pace (2009) 방식의 직·간접 효과 분해를 수행하였다(Table 4-14).
...
*Note: N=3,664 (229개 시군구 × 2009–2024, 1기 시차). 행표준화(row-standardized) Queen contiguity W 적용. Only selected key variables are shown. SDM estimated via 2SLS with WX as instruments. 유의수준: *** p<0.001, ** p<0.01, * p<0.05.*

**Table 4-14. SDM 직·간접 효과 분해 (LeSage-Pace)**

| Variable | Direct Effect | Indirect (Spillover) | Total Effect |
|:---|---:|---:|---:|
| ln(PageRank+1), lag1 | 3511.32*** | 5903.08** | 9414.40*** |
| Youth Ratio | 1.6124 | -1.0525 | 0.5599 |
| Extinction Risk Index | 15.9861 | -40.4899 | -24.5038 |

SDM 추정 결과, 공간 자기회귀 계수($\rho$)는 -0.0696으로 통계적으로 유의하지 않게 나타났으며($p>0.05$), 이는 Two-way FE가 공간적 의존성을 상당 부분 흡수하였기 때문으로 판단된다. 그러나 PageRank의 직접효과는 3,511로 여전히 강하게 유의하여 인구 흡인력을 설명하는 핵심 변수임을 재확인하였다. 효과 분해(Table 4-14) 결과, `ln(PageRank)`의 직접효과는 3,511로 나타났으며, 간접효과(5,903) 역시 유의하게 나타나 특정 지역이 네트워크 허브로 성장할 때 주변 지역으로도 긍정적인 파급효과가 발생함을 시사한다. 반면, 인접 지역의 PageRank가 높아질 경우 자지역의 순이동률이 감소하는 경쟁 효과($W\cdot\text{PageRank} = -293.6$)도 일부 관찰되었다.
...
*Note: N=3,664 (229개 시군구 × 2009–2024, 1기 시차). 각 모형은 중심성 지표만 다르고 통제변수는 동일한 Two-way FE 모형. In-degree의 부호 반전(-)은 단순 유입량(In-degree)이 많은 것이 실질적인 인구 흡인력 향상으로 이어지지 않음을 의미하며, PageRank가 포착하는 질적 네트워크 위상의 중요성을 방증한다. 유의수준: *** p<0.001, ** p<0.01, * p<0.05.*
...
*Note: N=3,664. 모든 W는 행표준화(row-standardized) 적용. Queen W는 직접 intersection 기반 인접성, KNN-5는 최근접 5개 지역, IDW는 100km 이내 역거리 가중치. 세 가지 W 모두에서 PageRank 직접효과가 강하게 유의하여 결과의 강건성을 확인하였다. 유의수준: *** p<0.001, ** p<0.01, * p<0.05.*
