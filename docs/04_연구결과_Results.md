...
## 4.5 연령대별 인구이동 네트워크의 구조적 이질성 (RQ3)
...
지역 간 인구이동은 인접 지역의 경제·사회적 여건에 영향을 받는 공간적 파급효과(Spatial Spillover)를 동반할 수 있다. 앞서 4.3.1절에서 확인한 바와 같이 Two-way FE 모형의 잔차에서 강한 공간적 자기상관이 발견되지는 않았으나, 독립변수의 공간 시차항($WX$)이 종속변수에 미치는 직접적인 파급효과를 통제하고 모형의 강건성을 검증하기 위해 공간더빈모형(Spatial Durbin Model, SDM)을 추정하였다(Table 4-9). 공간 가중치 행렬(W)은 직접 인접성(Queen contiguity) 기준을 적용하였으며, LeSage and Pace (2009) 방식의 직·간접 효과 분해를 수행하였다(Table 4-10).
...
|:---:|:---|---:|
| 1 | 노후주택비율 (house_age) | 3.662 |
| 2 | 네트워크 중심성 (pagerank_lag1) | 3.434 |
| 3 | 보육시설 (childcare_pk) | 3.244 |
| 4 | 인구밀도 (pop_density) | 2.757 |
| 5 | 서울까지의 거리 (seoul_dist_km) | 1.976 |
| 6 | 인구 규모 (ln_pop) | 1.558 |
| 7 | 합계출산율 (fertility) | 1.342 |
| 8 | 인구 천명당 의사수 (doctor_per1000) | 1.325 |
| 9 | 고령화율 (aging_ratio) | 1.258 |
| 10 | 청년비율 (youth_ratio) | 0.965 |

분석 결과(Table 4-17), **노후주택비율(house_age)**이 평균 SHAP Value 3.662로 1위를 차지하였다. 이는 주거 환경의 물리적 쾌적성이 인구 유입에 미치는 영향이 매우 크게 작동함을 의미한다. 이어 **네트워크 중심성(pagerank_lag1, 3.434)**이 2위를 기록하여, 선형 패널모형(FE)에서 확인한 허브 지위의 흡인력이 비선형 공간에서도 가장 강력한 결정요인 중 하나로 재확인되었다. 3위는 보육시설(3.244)로 생활 SOC의 중요성을 입증하였다. 반면 고용률, 사업체수 등 전통적인 경제 변수들은 Top 10에 포함되지 못해, 최근의 인구이동이 일자리보다는 정주 여건(주거·보육·의료) 중심으로 재편되고 있음을 시사한다.
