...
![Figure 4-6. Annual Change in Network Concentration (HHI)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_6_hhi_trend.png)
...
![Figure 4-7. Annual Change in Freeman Centralization](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_7_freeman_trend.png)
...
![Figure 4-4. Louvain Community Detection Map (2008)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_4_louvain_2008.png)
...
![Figure 4-5. Louvain Community Detection Map (2025)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_5_louvain_2025.png)
...
지역 간 인구이동은 인접 지역의 경제·사회적 여건에 영향을 받는 공간적 파급효과(Spatial Spillover)를 동반한다. 이를 확인하기 위해 Two-way FE 모형의 잔차에 대해 Moran's I 검정과 LM 검정을 수행한 결과, 고정효과가 공간 구조를 상당 부분 흡수하여 잔차의 공간 자기상관은 약하게 나타났다. 그러나 본 연구는 인구이동의 본질적인 공간적 파급 경로를 규명하기 위해 공간더빈모형(Spatial Durbin Model, SDM)을 추정하였다(Table 4-15). 공간 가중치 행렬(W)은 직접 인접성(Queen contiguity) 기준을 적용하였으며, LeSage and Pace (2009) 방식의 직·간접 효과 분해를 수행하였다(Table 4-16).

**Table 4-15. 공간더빈모형(SDM) 추정 결과**
...
**Table 4-16. SDM 직·간접 효과 분해 (LeSage-Pace)**
...
SDM 추정 결과, 공간 자기회귀 계수($\rho$)는 0.788($p<0.05$)로 유의하게 나타나, 인접 지역의 순이동률 증가가 자지역의 순이동률을 견인하는 강한 공간적 파급효과를 확인하였다. 효과 분해(Table 4-16) 결과, `ln(PageRank)`의 직접효과(2,798)보다 간접효과(5,903)가 약 2배 크게 나타났다. 이는 특정 지역이 네트워크 허브로 성장할 때, 해당 지역뿐만 아니라 인접한 주변 지역으로도 긍정적인 인구 유입 파급효과(Positive Spillover)가 크게 발생함을 시사한다. 반면, 인접 지역의 PageRank가 높아질 경우 자지역의 순이동률이 감소하는 경쟁 효과($W\cdot\text{PageRank} = -293.6$)도 일부 관찰되었다.
...
**Table 4-19. 수도권/비수도권 이질성 분석 (Two-way FE)**
...
**Table 4-20. 도시 규모별 이질성 분석 (Two-way FE)**
...
**Table 4-21. 시계열 구조변화 검정 (Chow Test)**
...
Table 4-19의 결과, 수도권(서울·인천·경기)에서는 PageRank의 효과가 강하게 유의($p<0.01$)한 반면, 비수도권에서는 유의하지 않았다($p=0.103$). Chow 검정 결과($F=9.67, p<0.01$) 두 집단 간 구조적 차이가 확연히 존재하였다. 또한 도시 규모별 분석(Table 4-20)에서도 시(市) 지역(2,279***)과 특광역시 자치구(1,624*)에서는 허브 효과가 유의했으나, 군(郡) 단위 농어촌 지역에서는 유의미한 정(+)의 효과가 나타나지 않았다. 이는 네트워크 중심성의 인구 흡인 효과가 대도시 및 수도권에 국한된 불균형적 기제임을 강력히 시사한다. 

더불어 Table 4-21의 시계열 구조변화 검정(Chow Test) 결과, 2기 신도시 입주가 본격화된 2015년($F=53.94, p<0.01$)과 COVID-19 팬데믹이 시작된 2020년($F=46.69, p<0.01$)을 전후로 인구이동 네트워크의 구조적 변화가 발생했음이 통계적으로 입증되었다.

### 4.4.2 대체 지표 및 가중치 행렬 강건성 검증

**Table 4-22. 대체 중심성 지표 강건성 검증 (Two-way FE)**
...
**Table 4-23. 대체 공간 가중치 행렬 강건성 검증 (SDM)**
...
또한 Table 4-23에서 보듯, 인접 지역 5개를 연결한 KNN-5와 100km 이내 역거리 가중치(IDW)를 적용한 SDM 추정 결과에서도 공간 자기회귀 계수($\rho$)와 PageRank의 직접효과가 모두 기준 모형(Queen W)과 동일한 부호로 강하게 유의($p<0.01$)하여, 본 연구의 공간 계량 분석 결과가 가중치 행렬의 형태에 의존하지 않는 매우 강건한 결과임을 확인하였다.
