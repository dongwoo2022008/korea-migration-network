...
분석 결과, 커뮤니티 탐지는 지리적 행정구역 단위가 아닌 실제 이동 흐름에 기반한 기능적 광역권 형성을 명확히 보여준다. 인구이동 네트워크는 밀도 0.95 이상의 거의 완전 그래프(Complete graph) 형태의 고밀도 네트워크 특성을 가지며, 이로 인해 Modularity는 0.44~0.50 수준의 안정적인 범위에서 도출되었다. 

Table 4-9는 주요 5개 연도(2009, 2012, 2016, 2020, 2025)의 커뮤니티 개수와 Modularity 지수를 정리한 것이다. 분석 기간 동안 커뮤니티 수는 6~7개로 일관되게 유지되었으며, Modularity 역시 0.444~0.498 사이에서 안정적인 추세를 보였다. 이는 인구이동의 구조적 패턴이 급격한 변동보다는 장기적으로 일관된 광역적 흐름을 유지하고 있음을 시사한다.

**Table 4-9. 연도별 커뮤니티 수 및 Modularity (인구이동 OD 네트워크 기반)**
...
*Note: Community detection performed using the Louvain algorithm applied to the directed/undirected migration OD network (in-/out-flow weighted). Edge weight = total migration flow. Modularity computed excluding self-loops. Node count adjusted to 229 municipalities in the final panel.*
...
2025년 기준 7개 커뮤니티의 구성을 구체적으로 살펴보면(Table 4-9b 및 Figure 4-5b), 단순한 지리적 행정 권역이 아닌 실제 이동 흐름이 만드는 기능적 광역권으로 재편되었음을 확인할 수 있다. 구체적으로 수도권(서울·경기 일부)과 인천권이 분리되어 독자적인 이동 권역을 형성하고 있으며, 충청권은 강원권과 결합하여 중부권 광역 벨트를 형성하였다. 영남권의 경우 동남권(부산·경남·울산)과 대경권(대구·경북)으로 명확히 분리되는 구조가 관찰된다.
...
*Note: 2025 migration community structure detected by Louvain algorithm (Modularity=0.444, 7 communities). Communities represent functional mega-regions driven by actual migration flows rather than strict administrative boundaries.*
