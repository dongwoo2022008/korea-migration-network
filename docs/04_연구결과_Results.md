...
앞선 분석에서 확인된 COVID-19 한계효과(§4.5.3, Figure 4-13)는 PageRank의 순이동률 결정력이 팬데믹 전(3,497)에서 팬데믹 기간(3,095)을 거쳐 엔데믹 이후(2,368)로 단계적으로 감소함을 보여주었다. 또한 Chow 구조변화 검정(§4.5.1, Table 4-13)은 2015년과 2020년을 전후로 회귀 구조가 통계적으로 유의하게 변화하였음을 입증하였다. 이러한 기존 분석 결과는 지역 흡인력의 결정구조가 고정적이지 않고 시간에 따라 변화하고 있다는 RQ5의 핵심 가설을 지지한다. 본 절에서는 이를 체계적으로 검증하기 위해 Chow 검정이 제시한 구조변화 시점(2015·2020년)을 기준으로 전체 관측 기간을 3개 시기로 분할하여 Two-way 고정효과 모형을 각각 추정하고, 연도별 횡단면 회귀를 통해 PageRank 결정력의 시계열 궤적을 시각화한다.
...
*주: 괄호 안은 HC1 이분산 강건 표준오차. \*p<0.05, \*\*p<0.01, \*\*\*p<0.001. Two-way FE(지역·연도 고정효과) 적용.*
...
RQ5 분석 결과를 종합하면, **지역 흡인력의 결정구조는 2006~2025년 동안 '네트워크 허브 중심(2010년대)'에서 '주거환경·의료 인프라·삶의 질 중심(2020년대)'으로 점진적으로 이동하고 있음**이 확인된다. 구체적으로, 허브 지위(PageRank)의 결정력은 2013~2014년 최고점 이후 장기적으로 약화되는 추세이며(Figure 4-16), 팬데믹 이후에는 소멸위험지수와 의료 인프라의 중요도가 상대적으로 부상하였다(Table 4-19). 이러한 결정구조의 시간적 이동은 RQ2(허브 효과)와 RQ4(주거환경·생활SOC 1위)를 시간축으로 통합하는 논문 전체의 핵심 서사를 구성한다.
