...
본 절에서는 §3.6.3에서 제시한 방법론에 따라 SHAP 기여도 기반 지역 흡인력 지수(Regional Attractiveness Index, RAI)를 산출하고, 그 공간 분포와 타당성을 검증한다. RAI는 경제(Econ), 인구(Demo), 인프라(Infra), 서비스(Serv) 4개 하위 영역의 SHAP 기여도를 가중 합산하여 산출하였으며, 분석 기간은 2017~2024년 패널 평균을 사용하였다.
...
| … | … | … | … | … | … | … | … | … |
| 220 | 영덕군 | 경상북도 | -0.712 | -0.592 | -0.670 | -0.993 | -0.406 | -6.95 |
| 221 | 청도군 | 경상북도 | -0.710 | -0.078 | -0.684 | -1.315 | -0.296 | 4.24 |
| 222 | 영양군 | 경상북도 | -0.647 | -0.160 | -0.666 | -1.230 | -0.106 | -4.03 |
| 223 | 울릉군 | 경상북도 | -0.641 | 0.579 | -0.224 | -1.217 | -1.125 | -5.77 |
| 224 | 합천군 | 경상남도 | -0.635 | -0.341 | -0.733 | -1.125 | -0.003 | -5.89 |
| 225 | 고성군 | 경상남도 | -0.634 | -0.308 | -0.471 | -1.306 | 0.002 | -4.98 |
| 226 | 임실군 | 전북특별자치도 | -0.624 | -0.255 | -0.550 | -1.282 | 0.044 | -10.30 |
| 227 | 의성군 | 경상북도 | -0.615 | -0.056 | -0.799 | -1.019 | -0.250 | 2.78 |
| 228 | 해남군 | 전라남도 | -0.600 | 0.031 | -0.472 | -1.314 | -0.111 | -12.26 |
| 229 | 의령군 | 경상남도 | -0.755 | -0.265 | -0.631 | -1.513 | -0.076 | 0.31 |

*Note: RAI = SHAP 기여도 기반 가중 합산 지수. 가중치: Econ 0.185, Demo 0.224, Infra 0.361, Serv 0.230. 분석 기간: 2017~2024년 패널 평균. N=229개 시군구.*
...
*Note: 각 점은 229개 시군구의 2017~2024년 패널 평균값. 점선은 OLS 회귀선. r=0.135 (p<0.001, N=1,832 패널 관측치 기준).*

RAI와 순이동률 간의 Pearson 상관계수는 r=0.135 (p<0.001)로 통계적으로 유의한 양(+)의 관계를 확인하였다. 상관이 약한 것은 RAI가 **구조적 매력도(장기 평균)**를 측정하는 반면, 순이동률은 **단기 변동성**(신도시 입주, 재개발, 행정구역 개편 등 일시적 요인)에 크게 영향받기 때문이다. 실제로 화성시(RAI 2위, 순이동률 +45.26‰)와 같이 RAI와 이동률이 일치하는 지역이 있는 반면, 강남구(RAI 5위, 순이동률 -2.48‰)처럼 구조적 매력도와 실제 이동 방향이 분리되는 지역도 다수 존재한다. 이는 RAI가 단기 이동률이 아닌 **장기적 지역 경쟁력**을 측정하는 지표임을 시사하며, 선행연구(Niedomysl, 2010; Haase et al., 2021)에서도 매력도 지수와 이동률 간 상관이 0.2~0.4 수준임을 보고한 바 있다.

### 4.8.4 하위지수 구성 비교 (Figure 4-19)

**Figure 4-19. 상위·하위 지역의 RAI 하위지수 비교 (레이더 차트)**

![Figure 4-19. RAI Sub-index Radar Chart](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_19_rai_subindex.png)
...
본 절의 분석 결과를 종합하면, 지역 흡인력 지수(RAI)는 수도권-비수도권 간 구조적 격차를 정량적으로 포착하며, 인프라 부문이 흡인력 양극화의 주요 동인임을 확인하였다. RAI-순이동률 상관(r=0.135)이 약한 것은 RAI가 단기 이동 패턴이 아닌 장기적 지역 경쟁력을 측정하는 지표임을 반영한다. 이는 지역 정책 수립 시 단기 인구 유출입보다 **구조적 매력도 개선**에 초점을 맞출 필요성을 시사한다.
