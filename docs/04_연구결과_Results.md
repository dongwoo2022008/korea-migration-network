...
|:---|---:|---:|---:|
| $\rho$ (Wy) | 0.7879** | (0.3712) | 0.0338 |
| ln(PageRank+1), lag1 | 2250.2520*** | (785.7884) | 0.0042 |
...
*Note: N=3552, $R^2=0.1264$. Only selected key variables are shown. SDM estimated via 2SLS with WX as instruments.*

**Table 4-14. SDM 직·간접 효과 분해 (LeSage-Pace)**

| Variable | Direct Effect | Indirect (Spillover) | Total Effect |
|:---|---:|---:|---:|
| ln(PageRank+1), lag1 | 2797.9126 | 5903.0750 | 8700.9876 |
| Youth Ratio | 1.6124 | -1.0525 | 0.5599 |
| Extinction Risk Index | 15.9861 | -40.4899 | -24.5038 |

SDM 추정 결과, 공간 자기회귀 계수($\rho$)는 0.788($p<0.05$)로 유의하게 나타나, 인접 지역의 순이동률 증가가 자지역의 순이동률을 견인하는 강한 공간적 파급효과를 확인하였다. 효과 분해(Table 4-14) 결과, `ln(PageRank)`의 직접효과(2,798)보다 간접효과(5,903)가 약 2배 크게 나타났다. 이는 특정 지역이 네트워크 허브로 성장할 때, 해당 지역뿐만 아니라 인접한 주변 지역으로도 긍정적인 인구 유입 파급효과(Positive Spillover)가 크게 발생함을 시사한다. 반면, 인접 지역의 PageRank가 높아질 경우 자지역의 순이동률이 감소하는 경쟁 효과($W\cdot\text{PageRank} = -293.6$)도 일부 관찰되었다.
...
**Table 4-21. 대체 공간 가중치 행렬 강건성 검증 (SDM)**

| 가중치 행렬 | $\rho$ (공간 자기회귀) | SE | p | ln(PageRank) 직접효과 | SE | p |
|:---|---:|---:|---:|---:|---:|---:|
| Queen W (기준) | 0.8960*** | (0.0910) | 0.0000 | 1098.54*** | (148.60) | 0.0000 |
| KNN-5 W | 1.1896*** | (0.1386) | 0.0000 | 1438.20*** | (190.23) | 0.0000 |
| IDW W (100km) | 6.9349*** | (1.9168) | 0.0003 | 690.69*** | (191.68) | 0.0003 |

또한 Table 4-21에서 보듯, 인접 지역 5개를 연결한 KNN-5와 100km 이내 역거리 가중치(IDW)를 적용한 SDM 추정 결과에서도 공간 자기회귀 계수($\rho$)와 PageRank의 직접효과가 모두 기준 모형(Queen W)과 동일한 부호로 강하게 유의($p<0.01$)하여, 본 연구의 공간 계량 분석 결과가 가중치 행렬의 형태에 의존하지 않는 매우 강건한 결과임을 확인하였다.
