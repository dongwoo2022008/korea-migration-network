...
네트워크 집중도를 PageRank 기준 상위 10% 점유율로 산정한 결과, 2006년 28.3%에서 2014년 26.3%로 소폭 하락한 후 2018~2019년에 28.2%로 일시 반등하였으며, 2025년에는 27.0% 수준을 유지하였다. HHI(Herfindahl-Hirschman Index)는 2006년 0.00765에서 2014년 0.00706으로 하락한 후 2018~2019년 0.00756로 반등하였으며 2025년에는 0.00722로 안정화되었다. 특히 주목할 점은 허브의 안정성(Hub Stability)이다. 2008년 상위 허브 지역들과 비교한 Jaccard 유사도 지수는 2010년 0.66에서 2018년 0.48, 2025년 0.42로 지속 하락하였다. 이는 기존 전통적 거점(예: 지방 광역시 중심구)들이 쇠퇴하고, 혁신도시 및 2기/3기 신도시를 품은 새로운 지역들이 새로운 허브로 부상하는 동태적 재편 과정이 활발히 일어났음을 시사한다.

### 4.2.4 커뮤니티 구조 변화

Louvain 알고리즘을 실제 시군구 간 인구이동 OD(Origin-Destination) 행렬에 적용하여 인구이동 네트워크의 커뮤니티 구조 변화를 분석하였다. 분석에는 전체 연령·성별 합산 이동자수(flow)를 엣지 가중치로 사용한 무방향 가중 네트워크를 구성하였으며, Louvain 알고리즘(가중 무방향, random_state 0~9 중 최댓값 적용, 자기루프 제외)을 적용하였다.

분석 결과, 커뮤니티 탐지는 지리적 행정구역 단위가 아닌 실제 이동 흐름에 기반한 기능적 광역권 형성을 명확히 보여준다. 인구이동 네트워크는 밀도 0.95 이상의 거의 완전 그래프(Complete graph) 형태의 고밀도 네트워크 특성을 가지며, 이로 인해 Modularity는 0.44~0.49 수준의 범위에서 도출되었다.

Table 4-9는 전 연도(2006~2025)의 커뮤니티 개수와 Modularity 지수를 정리한 것이다. 2006~2022년 기간에는 커뮤니티 수가 50~55개 수준에서 점진적으로 감소하는 추세를 보였으며, Modularity 역시 0.459~0.491 사이에서 완만하게 하락하였다. 그러나 2023년에는 커뮤니티 수가 31개로 급감하고 2024~2025년에는 20개 수준으로 더욱 축소되는 구조적 전환이 관찰되었다. 이 변화는 7개 시드(0, 1, 2, 42, 100, 200, 999) 모두에서 일관되게 재현되어 알고리즘의 확률적 변동이 아닌 실제 데이터 구조의 변화임이 확인되었다. 이는 팬데믹 이후 인구이동 패턴이 기존의 다극적 지역 분산 구조에서 소수 거점 중심의 통합 구조로 급격히 재편되고 있음을 시사한다.

**Table 4-9. 연도별 커뮤니티 수 및 Modularity (인구이동 OD 네트워크 기반, 자기루프 제거, n=229)**

| Year | Communities | Modularity | 비고 |
|:---:|:---:|:---:|:---:|
| 2006 | 53 | 0.490 | |
| 2007 | 53 | 0.486 | |
| 2008 | 53 | 0.488 | |
...
*Note: Community detection performed using the Louvain algorithm (undirected weighted graph, random_state=42, self-loops excluded). Edge weight = total migration flow (all ages, all genders aggregated). Node count = 229 municipalities (sgg_master.csv 기준). The structural break in 2023–2025 is robust across 7 random seeds (0, 1, 2, 42, 100, 200, 999).*

![Figure 4-4c. 인구이동 네트워크의 커뮤니티 모듈성 추이](../results/figures/fig4_4c_modularity_trend.png)

*Note: Trend of community modularity based on the actual migration OD network.*

2025년 기준 20개 커뮤니티의 구성을 살펴보면(Table 4-9b 및 Figure 4-5b), 2023년 이전(50개 내외)과 비교하여 커뮤니티 수가 대폭 감소한 가운데 기능적 광역권이 더욱 광역화·통합화된 형태로 재편되었음을 확인할 수 있다. 특히 2023년 이후의 구조변화는 팬데믹 이후 원격근무 정착, 수도권 집중 심화, 비수도권 인구 급감 등 복합적 요인에 의해 기존의 다극적 이동 구조가 소수 핵심 거점 중심으로 통합되는 과정을 반영한다.

**Table 4-9b. 2025년 커뮤니티별 주요 구성 시도**

| 커뮤니티 | 규모(n) | 주요 구성 (시도별 시군구 수) |
|:---|:---:|:---|
| 충청·강원권 | 53 | 강원 17, 충남 13, 충북 10, 경기 6 등 |
| 수도권 | 42 | 서울 25, 경기 16, 강원 1 |
| 호남권 | 40 | 전남 22, 전북 12, 광주 5, 충남 1 |
| 동남권 | 38 | 부산 16, 경남 16, 울산 5, 경북 1 |
| 대경권 | 30 | 경북 20, 대구 9, 경남 1 |
| 인천권 | 12 | 인천 10, 경기 2 |
| 제주권 | 2 | 제주 2 |

![Figure 4-5b. 2025년 인구이동 커뮤니티 구성](../results/figures/fig4_5b_louvain_2025.png)

*Note: 2025 migration community structure detected by Louvain algorithm (Modularity=0.441, 20 communities, random_state=42, self-loops excluded). The structural break from ~50 communities (2006–2022) to 20 communities (2024–2025) is robust across 7 random seeds.*

### 4.2.5 네트워크 집중도 공간 분포

네트워크 전체의 구조적 집중도 변화를 파악하기 위해 PageRank 기준 HHI와 상위 10% 점유율을 연도별로 산출하였다. 분석 결과, 상위 허브로의 이동 집중은 2006년(HHI=0.00765, Top10%=28.3%) 이후 2014년(HHI=0.00706, Top10%=26.3%)까지 소폭 완화된 후, 2018~2019년(HHI=0.00756, Top10%=28.2%)에 일시 반등하였으며, 2025년에는 HHI=0.00722, Top10%=27.0% 수준으로 안정화되었다. 이는 허브 지역의 흡인력이 시간에 따라 단조적으로 심화되기보다는 신도시 개발 주기와 연동하여 동태적으로 변화함을 시사한다. Figure 4-5는 HHI와 Top 10% 점유율의 연도별 추이를 보여준다.

![Figure 4-5. 인구이동 네트워크 집중도 추이 (2006–2025)](../results/figures/fig4_5_concentration_trend.png)

*Note: Left panel shows HHI (Herfindahl-Hirschman Index) based on PageRank share; right panel shows Top 10% PageRank share. Shaded area indicates COVID-19 pandemic period (2020–2021).*
...
### 4.2.6 공간 자기상관 분석 (Global Moran's I)

Table 4-10과 Figure 4-7은 2009~2024년 순이동률의 공간적 자기상관 추이를 보여준다. 분석의 일관성을 위해 국지적 분석(LISA)과 동일하게 229개 시군구 패널 기준 Queen contiguity 공간 가중치(행 표준화)를 적용하여 재산출하였다. Moran's I는 대부분의 연도에서 통계적으로 유의하지 않거나(n.s.) 약한 양(+)의 자기상관을 보였다.

구체적으로 2008년(I=0.142, p<0.01), 2010년(I=0.161, p<0.01), 2011년(I=0.101, p<0.05), 2012년(I=0.111, p<0.05) 시기에 통계적으로 유의한 양(+)의 공간적 자기상관이 관찰되었다. 이는 해당 시기 2기 신도시 입주 및 수도권 부동산 시장 확장과 맞물려 인구 유입이 특정 지역 주변으로 군집하는 현상이 일시적으로 강화되었음을 시사한다.

반면, 2013~2020년 대부분의 연도에서 Moran's I는 통계적으로 유의하지 않았다(n.s.). COVID-19 팬데믹 정점인 2021년(I=0.111, p<0.05)에는 오히려 유의한 양(+)의 자기상관이 재등장하였는데, 이는 팬데믹 기간 중 수도권 인접 지역으로의 이동 군집화가 일시적으로 강화되었음을 시사한다. 2022년(I=−0.096, p<0.10)에는 음(−)의 유의한 자기상관이 관찰되어 인구이동 패턴의 공간적 분산이 심화되었으며, 엔데믹 이후인 2024년(I=0.065, p<0.10)에는 다시 약한 양(+)의 자기상관이 회복되는 추세를 보였다.

**Table 4-10. 연도별 Global Moran's I 검정 결과 (2008~2025, Queen 가중치, n=229)**

| 연도 | Moran's I | E[I] | z-score | p-value (정규) | p_sim (순열) | 유의성 |
|:---:|---:|---:|---:|---:|---:|:---:|
| **2008** | **0.1423** | **−0.0044** | **2.7747** | **0.0055** | **0.007** | **\*\*\*** |
| 2009 | 0.0413 | −0.0044 | 0.8637 | 0.3877 | 0.151 | n.s. |
| **2010** | **0.1611** | **−0.0044** | **3.1307** | **0.0017** | **0.009** | **\*\*\*** |
| **2011** | **0.1006** | **−0.0044** | **1.9856** | **0.0471** | **0.024** | **\*\*** |
| **2012** | **0.1109** | **−0.0044** | **2.1804** | **0.0292** | **0.025** | **\*\*** |
| 2013 | 0.0290 | −0.0044 | 0.6322 | 0.5272 | 0.213 | n.s. |
| 2014 | 0.0374 | −0.0044 | 0.7911 | 0.4289 | 0.156 | n.s. |
| 2015 | 0.0095 | −0.0044 | 0.2634 | 0.7922 | 0.361 | n.s. |
| 2016 | 0.0390 | −0.0044 | 0.8197 | 0.4124 | 0.157 | n.s. |
| 2017 | 0.0338 | −0.0044 | 0.7216 | 0.4705 | 0.239 | n.s. |
| 2018 | 0.0448 | −0.0044 | 0.9298 | 0.3525 | 0.162 | n.s. |
| 2019 | −0.0107 | −0.0044 | −0.1197 | 0.9048 | 0.489 | n.s. |
| 2020 | 0.0173 | −0.0044 | 0.4104 | 0.6815 | 0.313 | n.s. |
| **2021** | **0.1106** | **−0.0044** | **2.1743** | **0.0297** | **0.027** | **\*\*** |
| **2022** | **−0.0960** | **−0.0044** | **−1.7318** | **0.0833** | **0.054** | **\*** |
| 2023 | −0.0667 | −0.0044 | −1.1779 | 0.2388 | 0.103 | n.s. |
| **2024** | **0.0651** | **−0.0044** | **1.3142** | **0.1888** | **0.088** | **\*** |
| 2025 | −0.0158 | −0.0044 | −0.2151 | 0.8297 | 0.425 | n.s. |

*Note: Moran's I statistics were estimated using row-standardized Queen contiguity weights (n=229 municipalities, self-loops excluded). E[I] = −1/(n−1) = −0.004386. Statistical significance assessed using 999 Monte Carlo permutations. \* p<0.10, \*\* p<0.05, \*\*\* p<0.01.*
...
### 4.2.7 국지적 공간 자기상관 분석 (LISA)

전국 수준의 Global Moran's I가 공간적 군집화의 전반적 추세를 보여준다면, 국지적 공간 자기상관 분석(Local Indicators of Spatial Association, LISA)은 어떤 지역이 구체적으로 군집의 핵심을 형성하는지를 규명한다. Figure 4-8~4-10은 2009년, 2021년(팬데믹 정점), 2024년의 LISA 군집 지도를 비교하여 보여준다. 분석에는 `analysis_dataset_FINAL_v4.csv`의 실제 순이동률(net_rate)을 사용하였으며, 앞선 Global Moran's I 분석과 완벽히 동일한 Queen contiguity 공간 가중치(행 표준화, 229개 시군구 패널 기준)와 999회 Monte Carlo 순열 검정(p<0.05)을 적용하였다.

**2009년(Figure 4-8):** 분석 초기에는 수도권 및 경기 남부 신도시 지역(HH 군집, n=10)이 핵심 인구 유입 군집을 형성하였으며, 비수도권 농산어촌 지역(LL 군집, n=30)이 인구 유출 군집으로 광범위하게 분포하였다. HL 군집(n=13)은 주변이 유출 지역인 가운데 독립적으로 유입을 기록하는 혁신도시·신도시 거점 지역으로 해석된다. 전체 유의 군집 지역은 60개(전체의 25.0%)이며, 동일 Queen 가중치 기준 Global Moran's I=0.041(p_sim=0.151)로 통계적으로 유의하지는 않으나 양(+)의 공간 자기상관 방향성을 보였다.

**2021년(Figure 4-9):** COVID-19 팬데믹 정점 시기에는 HH 군집이 11개로 유지되었으며, LL 군집은 24개로 다소 축소되었다. HL 군집(n=13)은 2009년과 동일한 수준을 유지하여 혁신도시·신도시 거점의 흡인력이 팬데믹 기간에도 지속되었음을 보여준다. Global Moran's I=0.111(p_sim=0.027, p<0.05)으로 통계적으로 유의한 양(+)의 공간 자기상관이 확인되었다. 전체 유의 군집 지역은 57개(23.5%)로 2009년과 유사한 수준이다.

**2024년(Figure 4-10):** 팬데믹 이후 HH 군집은 5개로 감소하고, LL 군집도 23개로 변화하였다. 전반적인 유의 군집 지역은 43개(17.6%)로 감소하여, 2024년 Global Moran's I=0.065(p_sim=0.088, p<0.10)의 약한 유의성과 일관된 결과를 보여준다. 이는 팬데믹 이후 원격근무 확산 및 생활권 변화로 인해 인구이동 패턴이 보다 분산된 형태로 전환되고 있음을 시사한다. HL 군집(n=11)의 지속적 존재는 특정 거점 지역의 독립적 흡인력이 구조적으로 유지되고 있음을 의미한다.
