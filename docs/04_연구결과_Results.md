...
| Variable | Direct Effect | SE | p | Indirect (Spillover) | SE | p | Total Effect | SE | p |
|:---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| ln(PageRank+1), lag1 | 3521.37** | (778.72) | 0.0010 | -672.24 | (973.49) | 0.4840 | 2849.13** | (989.17) | 0.0040 |
| W·ln(PageRank+1) | -464.00 | (1101.86) | 0.6737 | — | — | — | — | — | — |

*Note: N=3,345 (223개 시군구 × 2009–2024, 결측 제거 후). 효과분해는 LeSage & Pace (2009) 방식, 표준오차는 부트스트랩 1,000회. 행표준화 Queen W 적용. 유의수준: ** p<0.01, * p<0.05.*

SDM 추정 결과, 공간 자기회귀 계수($\rho$)는 -0.0696으로 통계적으로 유의하지 않게 나타났으며($p=0.186$), 이는 Two-way FE가 공간적 의존성을 상당 부분 흡수하였기 때문으로 판단된다. LeSage-Pace 효과분해(Table 4-14) 결과, `ln(PageRank)`의 **직접효과는 3,521**($p<0.01$)로 강하게 유의하여 자지역의 네트워크 중심성이 인구 흡인력을 결정하는 핵심 요인임을 재확인하였다. 반면, **간접효과(공간 파급효과)는 -672로 통계적으로 유의하지 않아**($p=0.484$), 행표준화 W 기반 재추정에서는 인접 지역으로의 유의한 정(+)의 파급효과가 확인되지 않았다. 이는 $\rho$가 비유의한 상황에서 공간적 파급 경로가 약화됨을 의미하며, 인구 흡인력은 지역 자체의 네트워크 위상에 의해 주로 결정되는 **지역 내부 효과(Own-region Effect)** 중심임을 시사한다. **총효과(2,849**)는 직접효과보다 작아 수학적으로도 일관성을 유지한다.
