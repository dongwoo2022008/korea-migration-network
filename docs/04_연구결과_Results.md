...
본 장에서는 제3장에서 제시한 분석 방법론에 따라 수행된 실증 분석 결과를 제시한다. 먼저 4.1절에서는 패널 데이터의 기술통계 및 다중공선성 진단 등 기초 진단 결과를 확인한다. 4.2절에서는 인구이동 네트워크의 거시적 진화와 구조적 변화(RQ1)를 시계열로 분석한다. 4.3절에서는 청년·중장년·고령 3개 연령군별 네트워크의 구조적 이질성(RQ3)을 비교한다. 4.4절과 4.5절에서는 공간 패널모형을 활용하여 지역 흡인력의 결정요인과 공간 파급효과(RQ2)를 추정하고 이질성 분석을 수행한다. 마지막으로 4.6절과 4.7절에서는 머신러닝 기법(XGBoost, LightGBM)과 SHAP 방법론을 적용하여 비선형 효과를 분석하고(RQ5), 최종적으로 산출된 지역 흡인력 지수(RAI)의 공간적 분포를 제시한다.
...
![Figure 4-1. 연도별 인구이동 네트워크 구조 (2008, 2012, 2016, 2020, 2025)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_1_network_structure.png)
...
![Figure 4-2. PageRank 상위 허브 지역의 중심성 변화 추세 (2008–2025)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_2_pagerank_trends.png)
...
![Figure 4-3. 네트워크 중심성 지표의 연도별 분포 변화 (2008–2025)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_3_centrality_boxplot.png)
...
네트워크 전체의 구조적 집중도 변화를 파악하기 위해 Freeman Centralization 지수와 PageRank 기준 허핀달-허쉬만 지수(HHI), 상위 10% 점유율을 연도별로 산출하였다. 분석 결과, 상위 허브로의 이동 집중은 2008년 이후 지속적으로 심화되는 추세를 보였다. PageRank 기준 HHI는 2008년 28.5%에서 2018년 29.1%로 정점을 기록하였으며, 이후 소폭 등락을 거듭하다 2025년 27.9% 수준을 보였다. 이는 허브 지역의 흡인력이 시간이 지날수록 더욱 강화되고 있음을 시사한다.

![Figure 4-4. Annual Change in Network Concentration (HHI)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_6_hhi_trend.png)
...
![Figure 4-5. Annual Change in Freeman Centralization](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_7_freeman_trend.png)
...
![Figure 4-6. Louvain Community Detection Map (2008)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_4_louvain_2008.png)
...
![Figure 4-7. Louvain Community Detection Map (2025)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_5_louvain_2025.png)
...
![Figure 4-8. Moran Scatter Plot of Net Migration Rate](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_8_moran_scatter.png)
...
![Figure 4-9. LISA 군집 지도 (2008)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_9_lisa_2008.png)

![Figure 4-10. LISA 군집 지도 (2025)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_10_lisa_2025.png)
...
![Figure 4-11. Hub Stability: Jaccard Similarity of Top-20 PageRank Hubs](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_11_jaccard_trend.png)
...
본 절에서는 연령군별로 구성된 인구이동 네트워크의 구조적 특성을 비교하여 RQ3("연령대별 인구이동 네트워크는 서로 다른 구조적 특성을 보이는가?")에 답한다. 분석 대상은 청년(20–39세), 중장년(40–59세), 고령(60세 이상)의 3개 연령군이며, 2008~2025년 18년간의 229개 시군구 간 OD 데이터를 연령군별로 집계하여 네트워크 지표를 산출하였다(자기루프 제거, 229 시군구 통일 적용).

### 4.3.1 연령군별 네트워크 구조 비교 (2025년 기준)

Table 4-3은 2025년 기준 3개 연령군의 핵심 네트워크 지표를 비교한 것이다.

**Table 4-3. 연령군별 네트워크 구조 비교 (2025년)**

| 지표 | 청년(20–39) | 중장년(40–59) | 고령(60+) |
|:---|---:|---:|---:|
| 총이동량 (명) | 1,986,518 | 980,750 | 556,417 |
| 네트워크 밀도 | 0.827 | 0.767 | 0.721 |
| HHI (PageRank) | 0.00794 | 0.00667 | 0.00639 |
| Top 10% 점유율 (%) | 30.7 | 26.2 | 25.2 |
| 실질 커뮤니티 수 | 8 | 8 | 7 |
| Modularity | 0.365 | 0.439 | 0.431 |

*Note: 데이터 출처: age_network_metrics_allyears.xlsx (자기루프 제거·229 시군구 통일·Louvain random_state=42 적용). HHI = Herfindahl-Hirschman Index of PageRank distribution.*

분석 결과, 세 연령군 간에 뚜렷한 구조적 이질성이 확인되었다. 청년 네트워크는 총이동량(199만 명)이 가장 크고, 네트워크 밀도(0.827)와 HHI(0.00794), Top10% 점유율(30.7%)이 모두 가장 높아 소수 허브로의 집중이 가장 강하게 나타났다. 반면 Modularity(0.365)는 세 집단 중 가장 낮아, 청년 이동이 권역 경계를 넘나드는 장거리·수도권 집중형 패턴임을 시사한다. 중장년과 고령 네트워크는 밀도와 집중도가 상대적으로 낮고 Modularity가 높아(0.439, 0.431), 지역 내 또는 근거리 이동 중심의 권역 구조를 유지하는 것으로 나타났다.

### 4.3.2 연령군별 Top 5 허브 비교 (2025년)

Table 4-4는 2025년 기준 연령군별 PageRank 상위 5개 허브를 비교한 것이다.

**Table 4-4. 연령군별 PageRank Top 5 허브 (2025년)**
...
*Note: PageRank 기준 순위. 데이터 출처: age_top_hubs_allyears.xlsx.*

허브 구성에서 생애주기별 분화가 뚜렷하게 관찰된다. 청년 네트워크에서는 관악구(서울)가 2위를 차지하는 것이 특징적인데, 이는 서울대학교를 비롯한 대학 밀집 지역으로의 진학·사회초년생 이동을 반영한다. 반면 중장년과 고령 네트워크에서는 관악구가 Top5에서 사라지고, 고령 네트워크에서는 고양시가 3위로 부상한다. 고양시는 상대적으로 주거 안정성이 높은 1기 신도시(일산) 지역으로, 정주 지향적 고령 이동의 특성을 반영한다. 수원시·화성시·용인시는 세 연령군 모두에서 공통적으로 Top5에 포함되어 경기 남부권이 연령 보편적 흡인 거점임을 확인할 수 있다.

### 4.3.3 연령군별 시계열 추이 (2008~2025년)

Figure 4-3은 2008~2025년 동안 세 연령군의 총이동량, Top10% 점유율, Modularity의 시계열 변화를 보여준다.

![Figure 4-3. 연령군별 네트워크 구조 시계열 추이 (2008~2025)](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig_r3_1_age_timeseries.png)

*Note: (a) 총이동량(백만 명), (b) Top 10% 허브 점유율(%), (c) Modularity. 데이터: age_network_metrics_allyears.xlsx.*

시계열 분석에서 세 가지 핵심 패턴이 확인된다. 첫째, **이동 규모의 연령별 분화**가 심화되고 있다. 청년 총이동량은 2008년 254만 명에서 2025년 199만 명으로 22% 감소하였으며, 중장년도 126만 명에서 98만 명으로 감소하였다. 반면 고령 이동량은 50만 명에서 56만 명으로 유일하게 증가하여 고령화의 직접적 반영임을 보여준다.

둘째, **청년 네트워크의 집중도 심화**가 관찰된다. 청년의 Top10% 점유율은 2008년 29.9%에서 2018~2023년 31%대까지 상승한 후 2025년 30.7%를 기록하며 전반적 상승 추세를 보인다. Modularity는 2008년 0.398에서 2025년 0.365로 하락하여, 청년 이동이 권역 경계를 무너뜨리며 수도권 허브로 더욱 집중되는 구조로 변화하고 있음을 시사한다.

셋째, **중·고령 네트워크의 권역 구조 유지**가 확인된다. 중장년의 Modularity는 0.43~0.46 수준을 유지하고 있으며, 고령 Modularity는 2020년 전후로 오히려 상승하는 경향을 보여 팬데믹 이후 고령층의 근거리·지역 내 이동 경향이 강화되었음을 시사한다.

이러한 결과는 **인구이동이 단일 네트워크가 아닌 연령별로 분화된 다층 네트워크(Multi-layer Network)**로 구성되어 있음을 실증하며, 특히 청년층의 수도권 초집중 구조가 지방소멸의 핵심 네트워크적 기제임을 확인한다. 이는 지역 흡인력 정책이 연령군별로 차별화되어야 함을 강력히 시사한다.
...
### 4.4.1 패널 고정효과 모형 추정 결과

먼저 지역의 미관측 이질성과 거시적 경제 충격을 통제하기 위해 개체(Entity) 및 시간(Time) 고정효과를 모두 포함하는 Two-way Fixed Effects (FE) 모형을 추정하였다. Table 4-12는 OLS, 개체 고정효과(Entity FE), 양방향 고정효과(Two-way FE) 모형의 비교 결과를 보여준다. Hausman 검정 결과($\chi^2(15)=34.62, p=0.003$), 고정효과 모형이 확률효과 모형보다 적합한 것으로 나타나 Two-way FE를 기본 모형으로 채택하였다.

**Table 4-12. 순이동률 결정요인 패널 회귀분석 (OLS vs FE)**
...
*Note: *** p<0.01, ** p<0.05, * p<0.1. Standard errors clustered by entity. Hausman Test: $\chi^2(15)=34.62 (p=0.003)$.*
...
지역 간 인구이동은 인접 지역의 경제·사회적 여건에 영향을 받는 공간적 파급효과(Spatial Spillover)를 동반할 수 있다. 이를 검증하기 위해 Two-way FE 모형의 잔차에 대해 Moran's I 검정과 LM 검정을 수행한 결과, 고정효과가 공간 구조를 상당 부분 흡수하여 잔차의 공간 자기상관은 약하게 나타났다. 본 연구는 인구이동의 공간적 파급 구조를 실증적으로 확인하기 위해 공간더빈모형(Spatial Durbin Model, SDM)을 추가로 추정하였다(Table 4-13). 공간 가중치 행렬(W)은 직접 인접성(Queen contiguity) 기준을 적용하였으며, LeSage and Pace (2009) 방식의 직·간접 효과 분해를 수행하였다(Table 4-14).

**Table 4-13. 공간더빈모형(SDM) 추정 결과**
...
**Table 4-14. SDM 직·간접 효과 분해 (LeSage-Pace, 행표준화 Queen W)**
...
SDM 추정 결과, 공간 자기회귀 계수($\rho$)는 -0.0696으로 통계적으로 유의하지 않게 나타났으며($p=0.186$), 이는 Two-way FE가 공간적 의존성을 상당 부분 흡수하였기 때문으로 판단된다. LeSage-Pace 효과분해(Table 4-14) 결과, PageRank의 영향은 **거의 전적으로 직접효과(3,519***)**에 기인하며, 간접효과(-511)는 작은 음(-)$의 값으로 나타났다. 이는 인접 지역이 네트워크 허브로 성장할 경우 자지역의 순유입이 소폭 감소하는 **경쟁·빨대 효과(competition/straw effect)**의 가능성을 시사하나, 공간 자기회귀 계수 $\rho$가 통계적으로 유의하지 않아(-0.070, $p=0.186$) 공간적 파급 자체는 제한적이다. 즉 양방향 고정효과 통제 이후에는 허브 지위의 인구 흡인력이 주로 **해당 지역 내부에서 작동하며, 공간적 누출은 약하다**. 이는 Table 4-21에서 세 가지 W 모두 $\rho$가 비유의로 나타난 결과와도 일관된다.

### 4.4.3 동적 패널모형 (Dynamic GMM) 검토

순이동률의 경로 의존성(Path Dependency)을 고려하여 종속변수의 시차변수($Y_{t-1}$)를 포함하는 동적 패널모형(Arellano-Bond 1차 차분 GMM)을 추정하였다. 추정 결과, $Y_{t-1}$의 계수는 1.25($p<0.01$)로 유의하여 인구이동의 강한 관성을 확인하였다. 그러나 핵심 변수인 `ln(PageRank)`의 계수가 음수(-30,425)로 반전되었으며, Sargan-Hansen 과잉식별 검정($p=0.000$)과 AR(2) 검정($p=0.000$)에서 귀무가설이 모두 기각되어 2기 및 3기 시차 도구변수의 유효성이 성립하지 않음이 확인되었다. 이는 차분 GMM의 전형적인 '약한 도구변수(Weak Instrument)' 문제에 기인한 것으로 판단되며, 따라서 본 연구는 동적 모형 대신 앞서 검증된 Two-way FE와 SDM을 최종 해석의 기준으로 삼는다.
...
![Figure 4-12. Marginal Effect of ln(PageRank+1) on Net Migration Rate by COVID-19 Period](https://raw.githubusercontent.com/dongwoo2022008/korea-migration-network/main/results/figures/fig4_12_covid_marginal.png)
...
**Table 4-17. 수도권/비수도권 이질성 분석 (Two-way FE)**
...
**Table 4-18. 도시 규모별 이질성 분석 (Two-way FE)**
...
**Table 4-19. 시계열 구조변화 검정 (Chow Test)**
...
Table 4-17의 결과, 수도권(서울·인천·경기)에서는 PageRank의 효과가 강하게 유의($p<0.01$)한 반면, 비수도권에서는 유의하지 않았다($p=0.103$). Chow 검정 결과($F=9.67, p<0.01$) 두 집단 간 구조적 차이가 확연히 존재하였다. 또한 도시 규모별 분석(Table 4-18)에서도 시(市) 지역(2,279***)과 특광역시 자치구(1,624*)에서는 허브 효과가 유의했으나, 군(郡) 단위 농어촌 지역에서는 유의미한 정(+)의 효과가 나타나지 않았다. 이는 네트워크 중심성의 인구 흡인 효과가 대도시 및 수도권에 국한된 불균형적 기제임을 강력히 시사한다. 

더불어 Table 4-19의 시계열 구조변화 검정(Chow Test) 결과, 2기 신도시 입주가 본격화된 2015년($F=53.94, p<0.01$)과 COVID-19 팬데믹이 시작된 2020년($F=46.69, p<0.01$)을 전후로 인구이동 네트워크의 구조적 변화가 발생했음이 통계적으로 입증되었다.

### 4.5.2 대체 지표 및 가중치 행렬 강건성 검증

**Table 4-20. 대체 중심성 지표 강건성 검증 (Two-way FE)**
...
**Table 4-21. 대체 공간 가중치 행렬 강건성 검증 (SDM, 행표준화 W)**
...
또한 Table 4-21에서 보듯, 행표준화 KNN-5와 IDW를 적용한 SDM 추정 결과에서도 PageRank의 직접효과가 모두 강하게 유의($p<0.001$)하여, 본 연구의 공간 계량 분석 결과가 가중치 행렬의 형태에 의존하지 않는 강건한 결과임을 확인하였다. 또한 공간 자기회귀 계수($\rho$)의 비유의성은 Two-way FE가 공간적 의존성을 상당 부분 흡수한 결과로 일관되게 해석된다.
...
**Table 4-22. 머신러닝 예측 모형 성능 평가 결과**
...
평가 결과(Table 4-22), 선형회귀(LR) 모형은 교차검증 단계에서 R²가 음수로 떨어지는 등 극심한 과적합과 성능 저하를 보인 반면, 트리 기반 앙상블 모형(XGBoost, LightGBM, CatBoost)들은 모두 0.4 이상의 우수한 Test R²를 기록하였다. 이는 인구이동 결정요인에 강한 비선형성과 변수 간 상호작용이 존재함을 강력히 시사한다.
...
**Table 4-23. SHAP 기반 지역 흡인력 변수 중요도 (Top 10)**
...
분석 결과(Table 4-23), 과거의 네트워크 중심성(PageRank)이 평균 SHAP Value 2.430으로 압도적인 1위 중요도를 차지했다. 이는 선형 패널모형에서 확인한 경로의존성과 허브 흡인력이 머신러닝의 비선형 공간에서도 가장 강력한 결정요인임을 교차 검증한 결과이다. 2위는 노후주택비율(2.032)로 주거 환경의 쾌적성이 인구 유입에 미치는 막대한 영향을 보여주었으며, 3위 보육시설(1.627), 5위 의료 인프라(1.306) 등 생활 SOC의 질적 수준이 전통적인 경제 변수(고용률 10위)보다 오히려 더 높은 중요도를 보였다.

## 4.7 지역 흡인력 지수(RAI) 산출 및 검증
