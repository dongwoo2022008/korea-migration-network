...
앞선 머신러닝 분석(RQ5)에서 도출된 최적 모형(CatBoost)의 SHAP 변수 중요도를 기반으로, 각 지역의 인구 흡인력을 종합적으로 평가하는 지역 흡인력 지수(Regional Attractiveness Index, RAI)를 산출하였다. 5개 하위 영역(네트워크, 생활SOC, 인구, 인프라, 경제)의 가중치는 해당 영역에 속한 변수들의 평균 절대 SHAP 값의 비율로 결정되었다(방법론 §3.6.3 참조).

**Table 4-20. 지역 흡인력 지수(RAI) 영역별 가중치**

| 영역 | 주요 포함 변수 | 가중치 (%) |
|:---|:---|---:|
| **네트워크 (Net)** | PageRank 중심성, 근접 중심성 | 54.4% |
| **생활SOC (Serv)** | 노후주택비율, 보육시설, 의사수, 문화시설 | 13.5% |
| **인구 (Demo)** | 인구규모, 청년비율, 합계출산율 | 11.4% |
| **인프라 (Infra)** | 인구밀도, 서울까지의 거리, 하수도보급률 | 11.1% |
| **경제 (Econ)** | 고용률, 재정자립도, 사업체수 | 9.6% |
| **Total** | - | **100.0%** |

산출 결과, PageRank가 포함된 **네트워크 영역(54.4%)**이 과반의 가중치를 차지하며 지역 흡인력의 핵심 동력임을 입증하였다. 이어 보육시설과 노후주택비율을 포괄하는 **생활SOC 영역(13.5%)**이 2위를 기록하였다. 반면 고용률 등 경제 영역(9.6%)의 비중은 가장 낮게 나타나, 인구 유입에 있어 단순한 경제적 기회보다는 네트워크 지위와 정주 환경이 더 결정적인 요인으로 작용함을 재확인하였다.
...
*Note: RAI = SHAP 기여도 기반 가중 합산 지수. 가중치: Net 54.4%, Serv 13.5%, Demo 11.4%, Infra 11.1%, Econ 9.6%. 분석 기간: 2017~2024년 패널 평균. N=228개 시군구(창원시는 2010년 통합시 신설로 독립변수 결측, RAI 산출 제외).*
